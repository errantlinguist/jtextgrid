...
public class TextGridFileReader<T> extends FileReader<TextGridFile<T>> {
	...
	private static enum Section {
		...
		INTERVAL_START(Pattern.compile("^\\s*intervals \\[(\\d+)\\]:\\s*")) {
			...
			protected <T> void handleMatch(final Matcher matcher,
				...
				reader.setEntryIndex(Integer.parseInt(matcher.group(1)));
		...
		TIER_START(Pattern.compile("^\\s*item \\[(\\d+)\\]:\\s*$")) {
			...
			protected <T> void handleMatch(final Matcher matcher,
				...
				reader.setTierIndex(Integer.parseInt(matcher.group(1)));
	...
	/**
	 * Sets the index of the next new {@link Entry} object.
	 * 
	 * @param id
	 *            The index of the entry.
	 */
	private void setEntryIndex(final int id) {
		entryIndex = id;
	...
	/**
	 * Sets the index of the next new {@link Tier} object.
	 * 
	 * @param id
	 *            The index of the tier.
	 */
	private void setTierIndex(final int id) {
		tierIndex = id;
